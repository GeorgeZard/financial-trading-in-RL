""" Optimizer implementations copied from https://github.com/nachiket273/lookahead_pytorch"""
...
def unitwise_norm(x: torch.Tensor):
    """
    copied from https://github.com/vballoli/nfnets-pytorch/blob/1b977fff2c89deb138b16d144fedba398f2ed344/nfnets/utils.py#L26
    :param x:
    :return:
    """
    if x.ndim <= 1:
...
def agc(optimizer: Optimizer, clipping=1e-2, eps=1e-3):
    for group in optimizer.param_groups:
        for p in group['params']:
            param_norm = torch.max(unitwise_norm(
                p), torch.tensor(eps).to(p.device))
            if p.grad is None:
                continue
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['weight_decay'] != 0:
                    p.add_(old, alpha=-group['weight_decay'] * group['lr'])
    ...
    def slow_step(self):
        for group, slow_weight in zip(self.param_groups, self.slow_weights):
            for param, weight in zip(group['params'], slow_weight):
                weight.add_((param - weight), alpha=self.alpha)
                param.copy_(weight)
        self.k_counter = 0
