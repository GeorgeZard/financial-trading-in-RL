...
class MarketExperiment7(TorchExperiment):

    def __init__(self, *args, **kwargs):
        super(MarketExperiment7, self).__init__(*args, **kwargs)
    ...
    def train(self, data, model_params=None, env_params=None, n_epochs=100, batch_size=32, ppo_clip=0.2,
              ...
              truncate_bptt=(5, 20), lookahead=False, beta=1, n_teachers=5, seed=0, kernel_parameters={}):
        ...
        model_dict = {'teacher 1': model1, 'teacher 2': model2, 'teacher 3': model3,
                      ...
                      'teacher 7': model7, 'student': model}
        ...
        for gi in tqdm(range(cur_train_step, n_epochs), smoothing=0., disable=not show_progress):
                ...
                for bi, (actions, old_log_prob, features, position, rews, old_logits_b, value_pred, advantage,
                    ...
                    for key, value in model_dict.items():
                        ...
                        if key != 'student':
                            ...
                            train_eval = value.train_eval(actions=new_actions, features=features,
                                                          position=new_positions)
                            # value_pred = to_np(train_eval['value'].squeeze()).astype(np.float32)
                            # advantage, returns = gae_engine.advantage(rews, value_pred)

                            policy_loss, pstats = ppo_categorical_policy_loss(new_actions, old_log_prob,
                                                                              Categorical(logits=old_logits_b),
                                                                              train_eval["pd"], advantage,
                                                                              ppo_clip=ppo_clip)
            ...
            if ((gi + 1) % validation_interval) == 0:
                ...
                del eval_dict
        # self.visualize_results(training_losses, 'Training loss')
    ...
    def get_new_actions_and_positions(self, logit, batch_size, max_episode_steps, num_actions):
        ...
        return positions.cuda(), actions.cuda()
